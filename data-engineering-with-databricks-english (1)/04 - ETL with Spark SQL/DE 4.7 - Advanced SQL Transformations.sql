...
SELECT value:device, value:geo:city , value:ecommerce, value:ecommerce:purchase_revenue_in_usd, value:items
FROM events_strings
where value:event_name = "finalize"

-- COMMAND ----------

CREATE OR REPLACE TEMP VIEW eventswithitems
AS
SELECT key, value:device, value:geo:city , value:ecommerce, value:ecommerce:purchase_revenue_in_usd, value:items, 
from_json(value:items, schema_of_json('[{"coupon":"NEWBED10","item_id":"M_STAN_Q","item_name":"Standard Queen Mattress","item_revenue_in_usd":940.5,"price_in_usd":1045.0,"quantity":1},{"coupon":"NEWBED10","item_id":"P_DOWN_S","item_name":"Standard Down Pillow","item_revenue_in_usd":107.10000000000001,"price_in_usd":119.0,"quantity":1}]')) AS items_json 
FROM events_strings
where value:event_name = "finalize" 

-- COMMAND ----------

CREATE OR REPlACE TEMP VIEW eventswithitemsexploded
AS
SELECT *, size(items_json), explode(items_json) AS item
FROM eventswithitems ;

SELECT *,
item.* 
FROM eventswithitemsexploded;

SELECT Key, City, Device,
collect_set(items_json.item_id),
flatten(collect_set(items_json.item_id)) 
FROM eventswithitemsexploded
GROUP BY Key,City, Device
ORDER BY Key 
...
DESCRIBE EXTENDED new_events_final;
--SHOW TABLES;
...
SELECT user_id, event_timestamp, event_name, size(items), explode(items) AS item
FROM events
WHERE size(items) > 2
order by size(items) DESC, user_id, event_timestamp;

--SELECT user_id, geo.* FROM events
...
SELECT collect_set(user_id) as allusers
FROM events
WHERE size(items) >= 1;


SELECT user_id, event_timestamp, event_name, collect_set(items.item_id) as collect_event_ids, flatten(collect_set(items.item_id)) as flatten_event_ids
FROM events
WHERE size(items) >= 1
GROUP BY user_id, event_timestamp, event_name
ORDER BY user_id, event_timestamp, event_name ;

CREATE OR REPLACE TEMP View MyTempView
AS
SELECT user_id,
  collect_set(event_name) AS event_history, -- array<string>
  collect_set(items.item_id) as collect_item_ids, --array<array<string>>
  flatten(collect_set(items.item_id)) as flatten_item_ids, --array<string>
  array_distinct(flatten(collect_set(items.item_id))) AS cart_history --array<string>

FROM events
GROUP BY user_id;

DESCRIBE  MyTempView
--DROP TABLE MyTempView
...
SELECT *, a.item.item_name as item_name2, a.item.price_in_usd as price2
FROM (
  SELECT *, explode(items) AS item 
  FROM sales) a
