...
SELECT COUNT(*) FROM sales_csv --WHERE order_id IS NOT NULL 
...
-- MAGIC %python
-- MAGIC DA.paths.sales_csv
-- MAGIC dbutils.fs.ls(DA.paths.sales_csv)

-- COMMAND ----------

DROP TABLE sales_csv_managed

-- COMMAND ----------

CREATE OR REPLACE TEMPORARY VIEW temp_sales USING CSV OPTIONS (
  path = '${DA.paths.sales_csv}',
  header = "true",
  delimiter = "|",
  mode = "FAILFAST" -- abort file parsing with a RuntimeException if any malformed lines are encountered
);
CREATE OR REPLACE TABLE sales_csv_managed 
AS
  SELECT *
  FROM temp_sales;

SELECT * FROM sales_csv_managed;
SELECT COUNT(1) FROM sales_csv_managed;
--DESCRIBE TABLE EXTENDED sales_csv_managed;
--DESCRIBE HISTORY sales_csv_managed
--SELECT * FROM sales_csv_managed
...
SELECT COUNT(1) FROM users_jdbc;
