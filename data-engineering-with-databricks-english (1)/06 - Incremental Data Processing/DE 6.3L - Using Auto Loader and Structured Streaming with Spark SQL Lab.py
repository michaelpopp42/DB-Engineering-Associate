...
df = spark.sql("show tables").select("tableName", "isTemporary")
display(df)
#Row(tableName="customers_raw_temp", isTemporary=False) in df.collect()
type(df.collect())
df.collect().count("*")
df.collect()
##type(Row(tableName="customers_raw_temp") in df.collect())
...
# MAGIC %sql
# MAGIC 
# MAGIC DESCRIBE EXTENDED customer_count_by_state;
# MAGIC DESCRIBE HISTORY customer_count_by_state

# COMMAND ----------

dbutils.fs.ls("dbfs:/mnt/dbacademy-users/michaelpopp42@gmail.com/data-engineering-with-databricks/database.db/customer_count_by_state")
#dbutils.fs.ls(f"{DA.paths.datasets}/retail-org/customers/")
