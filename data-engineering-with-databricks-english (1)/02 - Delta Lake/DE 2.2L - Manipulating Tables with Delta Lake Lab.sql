...
-- TODO
CREATE OR REPLACE TABLE beans ( name STRING, color STRING, grams FLOAT, delicious BOOLEAN)
...
-- MAGIC %python
-- MAGIC spark.conf.get("spark.databricks.delta.lastCommitVersionInSession")
-- MAGIC #spark.table("beans").select("name").show()
-- MAGIC #spark.table("beans").select("name").collect()
-- MAGIC #aset = set(row["name"] for row in spark.table("beans").select("name").collect())
-- MAGIC #aset.add("garbonzo")
-- MAGIC #aset
-- MAGIC #aset.add("garbonzo")
-- MAGIC #aset
-- MAGIC #aset.remove("garbonzo")
-- MAGIC #aset
-- MAGIC #aset.remove("garbonzo")
-- MAGIC #aset
-- MAGIC #rows = spark.table("beans").filter("name='pinto' OR grams >= 1000")
-- MAGIC #rows.first()
-- MAGIC #rows.count()
-- MAGIC #rowlist  = rows.collect()
-- MAGIC #rowlist[1]
-- MAGIC #for row in rowlist:
-- MAGIC #    for col in row:
-- MAGIC #        display(col)
-- MAGIC #columnList = rowlist[1].collect()
-- MAGIC #columnList
-- MAGIC #help(rowlist)
-- MAGIC #rowlist.count()
-- MAGIC #hist = spark.sql("DESCRIBE HISTORY beans")
-- MAGIC #display(hist)
