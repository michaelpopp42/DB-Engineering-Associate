...
FROM users_dirty
--where user_id is NOT NULL -- This has no effect because count eliminates the null value Anyway

-- This will return 918 rows becuase it includes the NULL value
--SELECT DISTINCT(user_id)
--FROM users_dirty
--where user_id is NOT NULL --this returns 917 rows
...
SELECT DISTINCT(*) FROM users_dirty -- includes rows that have nulls in any column
--WHERE user_id IS NULL;

--SELECT COUNT(*) FROM users_dirty;
--SELECT COUNT(DISTINCT(*)) FROM users_dirty; -- only counts rows that have no NULLs in any columns
--SELECT COUNT(*) FROM users_deduped;
--SELECT COUNT(DISTINCT(*)) FROM users_deduped;
...
-- above returns 917 rows
-- below returns 983
;SELECT user_id, user_first_touch_timestamp
FROM users_dirty
WHERE user_id IS NOT NULL;

-- below retruns the 66 duplicate rows
-- 983-66 = 917
WITH duplicate_users
...
-- All these duplicated users have one entry with an email and one withput an email
SELECT u.* 
FROM users_dirty u
INNER JOIN duplicate_users d
