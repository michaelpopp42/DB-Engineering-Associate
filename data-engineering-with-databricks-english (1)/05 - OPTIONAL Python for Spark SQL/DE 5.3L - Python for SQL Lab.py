...
import re
print(re.fullmatch("[A-Z][A-Z]","ABC"))
#display(preview_values(render_results=True))
preview_values(state="CA", render_results=True)
display(preview_values(state="CA", render_results=False))
"s7".upper()
...
def preview_values(state=None, render_results=False):
    ...
    if state is not None:
        assert re.fullmatch("[A-Z][A-Z]", state) != None, f"state={state} is invalid. Must be two upper case alphas."
