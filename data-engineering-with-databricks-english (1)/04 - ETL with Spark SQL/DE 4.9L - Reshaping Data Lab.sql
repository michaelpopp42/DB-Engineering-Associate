...
SELECT * FROM events;
  ...
  SELECT COUNT(*) from transactions; -- 10539
  SELECT COUNT(*) from events; --485824
...
SELECT * FROM
(
  SELECT e.user_id AS user, e.event_name AS event_name 
  FROM events e
)
PIVOT (
...
-- TODO
CREATE OR REPLACE VIEW clickpaths
AS
SELECT ep.*, t.*  
FROM transactions t
INNER JOIN events_pivot ep
ON t.user_id = ep.user;
...
SELECT sum(CAST(mattress AS INT)) num_mattress, sum(CAST(pillow AS INT)) num_pillow FROM sales_product_flags
