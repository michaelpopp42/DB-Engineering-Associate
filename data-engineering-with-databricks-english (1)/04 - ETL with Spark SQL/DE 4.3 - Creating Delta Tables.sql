...
-- MAGIC %python
-- MAGIC rawdir = DA.paths.datasets + '/ecommerce/raw/sales-historical'
-- MAGIC dbutils.fs.ls(rawdir)
...
(3, 1592504254634073, 42.0, '2020-06-18')
...
SELECT CAST(updated as bigint) AS tm, CAST(CAST(updated as bigint) AS TIMESTAMP), * FROM users_pii;
...
DESCRIBE TABLE EXTENDED purchases_shallow_clone

-- COMMAND ----------

-- MAGIC %python 
-- MAGIC files = dbutils.fs.ls('dbfs:/mnt/dbacademy-users/michaelpopp42@gmail.com/data-engineering-with-databricks/database.db/purchases_clone')
-- MAGIC display(files)
