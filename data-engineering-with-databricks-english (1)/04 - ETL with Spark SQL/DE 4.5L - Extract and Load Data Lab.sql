...
-- MAGIC rowsdf = spark.table("events_raw").select("timestamp")
-- MAGIC display(rowsdf)
-- MAGIC timestamps = set(row["timestamp"] for row in spark.table("events_raw").select("timestamp").limit(5).collect())
-- MAGIC assert timestamps == {1593880885085, 1593880886106, 1593880889174, 1593880889725, 1593880892303} , "rows are not correct"
